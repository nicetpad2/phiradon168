...
AUTO_THRESHOLD_TUNING = os.getenv("AUTO_THRESHOLD_TUNING", "False").lower() in (
    "true",
    "1",
    "yes",
)
...
from sklearn.preprocessing import (
    StandardScaler,
    OrdinalEncoder,
)  # Added OrdinalEncoder back as it might be used by some logic
...
try:
    ...
    try:
        ...
        logging.warning(
            f"   (Warning) ไม่สามารถตรวจสอบจำนวน GPU ของ CatBoost: {e_cb_gpu_check}"
        )
except ImportError:
    if AUTO_INSTALL_LIBS:
...
try:
    ...
    if AUTO_INSTALL_LIBS:
        ...
        try:
            subprocess.run(
                [sys.executable, "-m", "pip", "install", "psutil", "-q"], check=True
            )
            import psutil
...
try:
    ...
    if AUTO_INSTALL_LIBS:
        ...
        try:
            subprocess.run(
                [sys.executable, "-m", "pip", "install", "gputil", "-q"], check=True
            )
            import GPUtil
...
try:
    ...
    try:
        if torch.cuda.is_available():
            ...
            logging.info(
                "   (Info) PyTorch ไม่พบ GPU หรือ CUDA ไม่พร้อม. การเร่งความเร็วด้วย GPU จะถูกปิด."
            )
            USE_GPU_ACCELERATION = False
    except Exception as e_cuda:
        logging.warning("   (Warning) ไม่สามารถใช้งาน CUDA ได้ -- ปิด GPU Acceleration.")
...
# [Patch v6.2.3] Handle PyTorch import errors concisely
    ...
    logging.warning(
        f"(Warning) GPU acceleration disabled due to import error: {e_torch_import}"
    )
    USE_GPU_ACCELERATION = False
...
def print_gpu_utilization(context=""):  # pragma: no cover
    ...
    if USE_GPU_ACCELERATION and pynvml and nvml_handle:
        try:
            ...
            logging.warning(
                f"NVML Error during GPU monitoring: {e_gpu_mon}. Disabling pynvml monitoring."
            )
            if pynvml:
                try:
                    pynvml.nvmlShutdown()
            ...
            logging.warning(
                f"Unexpected error retrieving GPU stats: {e_gpu_mon_other}."
            )
            if pynvml:
                try:
                    pynvml.nvmlShutdown()
        ...
        gpu_util_str = "pynvml N/A"
        gpu_mem_str = "pynvml N/A"
    elif not USE_GPU_ACCELERATION:
        gpu_util_str = "Disabled"
        gpu_mem_str = "Disabled"

    if psutil:
        try:
            mem = psutil.virtual_memory()
            ram_str = (
                f"{mem.percent}% ({mem.used // 1024**2}MB / {mem.total // 1024**2}MB)"
            )
    ...
    logging.info(
        f"[{context}] GPU Util: {gpu_util_str} | Mem: {gpu_mem_str} | RAM: {ram_str}"
    )
...
def show_system_status(context=""):  # pragma: no cover
    ...
    if psutil:
        try:
            ...
            logging.error(
                f"Error getting RAM info (GPUtil func): {e_ram_mon}", exc_info=True
            )
    else:
        ram_str = "psutil N/A"
    ...
    if GPUtil:
        try:
            ...
            if gpus:
                ...
                for gpu in gpus:
                    gpu_str_list.append(
                        f"GPU {gpu.id} {gpu.name} | Load: {gpu.load*100:.1f}% | Mem: {gpu.memoryUtil*100:.1f}% ({gpu.memoryUsed:.0f}MB/{gpu.memoryTotal:.0f}MB)"
                    )
            else:
            ...
            logging.error(
                f"Error getting GPU info via GPUtil: {e_gpu_mon}", exc_info=True
            )
    else:
...
warnings.filterwarnings(
    "ignore", message="Using CPU via PyArrow to expand CUDF columns"
)
...
class DefaultConfig:
    ...
    DEFAULT_RISK_PER_TRADE: float = FUND_PROFILES.get(DEFAULT_FUND_NAME, {}).get(
        "risk", 0.01
    )
...
MIN_SIGNAL_SCORE_ENTRY = max(
    MIN_SIGNAL_SCORE_ENTRY_MIN, min(MIN_SIGNAL_SCORE_ENTRY_MAX, MIN_SIGNAL_SCORE_ENTRY)
)
...
ADAPTIVE_TSL_START_ATR_MULT = (
    1.5  # ATR multiplier from entry price to start Trailing Stop Loss
)
ADAPTIVE_TSL_DEFAULT_STEP_R = (
    0.5  # Default TSL step size (in R units) in normal volatility
)
...
MAX_WICK_RATIO_ENTRY = (
    0.75  # Maximum candle wick ratio allowed for entry (prevents spikes)
)
...
M1_ENTRY_CANDLE_RATIO_THRESH = (
    0.85  # (Not directly used in current logic, kept for potential future use)
)
M1_ENTRY_MACD_HIST_THRESH = (
    -0.1
)  # (Not directly used in current logic, kept for potential future use)
FAST_MA_PERIOD = 15  # [Patch v6.5.9] Fast MA period for fallback entry
SLOW_MA_PERIOD = 50  # [Patch v6.5.9] Slow MA period for fallback entry
...
try:
    SESSION_TIMES_UTC = (
        json.loads(session_env)
        if session_env
        else {"Asia": (22, 8), "London": (7, 16), "NY": (13, 21)}
    )
except Exception:
...
ENTRY_CONFIG_PER_FOLD = {
    ...
    0: {
        ...
        "gain_z_thresh": 0.3,
        "cooldown_sec": 0,
        "min_signal_score": MIN_SIGNAL_SCORE_ENTRY,
    },
    1: {
        "sl_multiplier": 2.0,
        "gain_z_thresh": 0.3,
...
logging.debug(
    f"Entry Config Per Fold (First Fold Example): {ENTRY_CONFIG_PER_FOLD.get(0, {})}"
)
...
KILL_SWITCH_MAX_DD_THRESHOLD = (
    0.30  # ↑ Relax kill switch ให้เลิก block ช้าลง (30% drawdown)
)
KILL_SWITCH_CONSECUTIVE_LOSSES_THRESHOLD = (
    5  # [Patch] Lower threshold for earlier soft cooldown
)
MAX_DRAWDOWN_THRESHOLD = 0.10  # [Patch v6.1.6] Tighter drawdown block threshold
logging.info(f"OMS Enabled: {OMS_ENABLED}")
logging.info(
    f"Kill Switch Enabled: {ENABLE_KILL_SWITCH} (DD > {KILL_SWITCH_MAX_DD_THRESHOLD*100:.0f}%, Losses > {KILL_SWITCH_CONSECUTIVE_LOSSES_THRESHOLD})"
)
logging.info(
    f"Max Drawdown Threshold (Block New Orders): {MAX_DRAWDOWN_THRESHOLD*100:.0f}%"
)
...
logging.info(
    f"Recovery Mode Enabled: Losses >= {RECOVERY_MODE_CONSECUTIVE_LOSSES}, Lot Multiplier: {RECOVERY_MODE_LOT_MULTIPLIER}"
)
...
REENTRY_MIN_PROBA_THRESH = (
    0.40  # ↓ ลดเงื่อนไขให้ยอมรับ ML probability ที่ต่ำกว่า (เพื่อ Re-entry บ่อยขึ้น)
)
logging.info(
    f"Re-Entry Enabled: {USE_REENTRY} (Cooldown: {REENTRY_COOLDOWN_BARS} bars, Threshold: {REENTRY_MIN_PROBA_THRESH})"
)
...
FORCED_ENTRY_ALLOWED_REGIMES = [
    ...
    "InsideBar",
    "Choppy",
]  # [Patch v5.6.4] Allow more patterns for FE
FE_ML_FILTER_THRESHOLD = 0.40  # ML probability threshold for filtering FE (if used)
ADAPTIVE_COOLDOWN_LOSS_MULTIPLIER = (
    0.2  # (Not directly used in FE, kept for potential future use)
)
forced_entry_max_consecutive_losses = (
    2  # Max consecutive FE losses before temporary disable
)
min_equity_threshold_pct = 0.70  # Min equity % of initial capital before disabling FE
logging.debug(
    f"Forced Entry Config: Bars>{FORCED_ENTRY_BAR_THRESHOLD}, Score>{FORCED_ENTRY_MIN_SIGNAL_SCORE:.2f}, MktConds:{FORCED_ENTRY_CHECK_MARKET_COND}, MaxLoss:{forced_entry_max_consecutive_losses}"
)
...
EQUITY_LOT_REDUCTION_THRESHOLD_PCT = (
    0.90  # Equity % vs peak before reducing lot (not used in current logic)
)
...
RSI_DRIFT_OVERRIDE_THRESHOLD = (
    0.65  # Threshold to ignore RSI scoring when drift is high
)
ATR_DRIFT_OVERRIDE_THRESHOLD = (
    0.25  # Threshold to enable gain-based exit on high ATR drift
)

# --- Dynamic Adjustment Configuration ---
logging.debug("Setting Dynamic Adjustment Configuration...")
DYNAMIC_GAINZ_DRIFT_THRESHOLD = (
    0.10  # Wasserstein threshold on Gain_Z to trigger adjustment
)
